"""
ORM model for Stock - separate from domain entity to avoid metaclass conflicts.
"""
...
class Stock(Base):
    """
    SQLAlchemy ORM model for Stock.
    ...
    # Dividend and corporate actions (JSON stored as text)
    ...
    # Calculated metrics
    trailing_dividend_yield = Column(Numeric(5, 4), nullable=True)
    annual_dividends = Column(Numeric(15, 4), nullable=True)
    ...
    def __repr__(self):
        return f"<Stock(id={self.id}, ticker={self.ticker}, exchange_id={self.exchange_id}, price={self.current_price})>"