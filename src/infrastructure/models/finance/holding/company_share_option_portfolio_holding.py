...
class CompanyShareOptionPortfolioHoldingModel(PortfolioHoldingsModel):
    ...
    company_share_option_portfolio_id = Column(Integer, ForeignKey("company_share_option_portfolios.id"), nullable=False)
    ...
    # Relationships
    company_share_option_portfolios = relationship(
        "src.infrastructure.models.finance.portfolio.company_share_option_portfolio.CompanyShareOptionPortfolioModel",
        back_populates="company_share_option_portfolio_holdings"
    )

    company_share_options = relationship(
        "src.infrastructure.models.finance.financial_assets.derivative.option.company_share_option.CompanyShareOptionModel",
        back_populates="company_share_option_portfolio_holdings"
    )

    __mapper_args__ = {
        "polymorphic_identity": "CompanyShareOptionPortfolioHoldings",
    }