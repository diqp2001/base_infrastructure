"""
IBKR Derivative Factor Repository - Interactive Brokers implementation for DerivativeFactor entities.

This repository handles derivative factor data from IBKR API,
applying IBKR-specific business rules before delegating persistence to local repository.
"""

from typing import Optional, List, Dict, Any
from datetime import date

from src.domain.ports.factor.finance.financial_assets.derivatives.derivative_factor_port import DerivativeFactorPort
from src.infrastructure.repositories.ibkr_repo.base_ibkr_factor_repository import BaseIBKRFactorRepository
from src.domain.entities.factor.finance.financial_assets.derivatives.derivative_factor import DerivativeFactor
from src.infrastructure.repositories.ibkr_repo.tick_types.ibkr_tick_mapping import IBKRTickFactorMapper, IBKRTickType


class IBKRDerivativeFactorRepository(BaseIBKRFactorRepository, DerivativeFactorPort):
    """
    IBKR implementation of DerivativeFactorPort.
    Handles derivative factor data acquisition from Interactive Brokers API and delegates persistence to local repository.
    """

    def __init__(self, ibkr_client, factory=None):
        """
        Initialize IBKR Derivative Factor Repository.
        
        Args:
            ibkr_client: Interactive Brokers API client
            factory: Repository factory for dependency injection (optional)
        """
        super().__init__(ibkr_client)
        self.factory = factory
        
    @property
    def entity_class(self):
        return DerivativeFactor

    @property
    def local_repo(self):
        """Get local derivative factor repository for delegation."""
        if self.factory:
            return self.factory._local_repositories.get('derivative_factor')
        return None

    # DerivativeFactorPort interface implementation (delegate to local repository)
    
    def get_by_id(self, entity_id: int) -> Optional[DerivativeFactor]:
        """Get derivative factor by ID (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_id(entity_id)
        return None

    def get_by_name(self, name: str) -> Optional[DerivativeFactor]:
        """Get derivative factor by name (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_name(name)
        return None

    def get_by_derivative_type(self, derivative_type: str) -> List[DerivativeFactor]:
        """Get derivative factors by derivative type (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_derivative_type(derivative_type)
        return []

    def get_by_underlying_symbol(self, symbol: str) -> List[DerivativeFactor]:
        """Get derivative factors by underlying asset symbol (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_underlying_symbol(symbol)
        return []

    def get_by_expiration_range(self, start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[DerivativeFactor]:
        """Get derivative factors by expiration date range (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_expiration_range(start_date, end_date)
        return []

    def get_by_group(self, group: str) -> List[DerivativeFactor]:
        """Get derivative factors by group (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_by_group(group)
        return []

    def get_all(self) -> List[DerivativeFactor]:
        """Get all derivative factors (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.get_all()
        return []

    def add(self, entity: DerivativeFactor) -> Optional[DerivativeFactor]:
        """Add derivative factor entity (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.add(entity)
        return None

    def update(self, entity: DerivativeFactor) -> Optional[DerivativeFactor]:
        """Update derivative factor entity (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.update(entity)
        return None

    def delete(self, entity_id: int) -> bool:
        """Delete derivative factor entity (delegates to local repository)."""
        if self.local_repo:
            return self.local_repo.delete(entity_id)
        return False

    def get_or_create_from_tick_type(self, tick_type: IBKRTickType, contract_data: Optional[Dict[str, Any]] = None) -> Optional[DerivativeFactor]:
        """
        Get or create a derivative factor from IBKR tick type.
        
        Args:
            tick_type: IBKR tick type enum (e.g., IBKRTickType.IMPLIED_VOLATILITY)
            contract_data: Optional contract details from IBKR API
            
        Returns:
            DerivativeFactor entity from database or newly created
        """
        try:
            # Get factor mapping configuration from IBKR tick type
            factor_mapping = IBKRTickFactorMapper.get_factor_mapping(tick_type)
            if not factor_mapping:
                print(f"No factor mapping found for tick type {tick_type}")
                return None
            
            # Check if factor already exists by name
            if self.local_repo:
                existing_factor = self.local_repo.get_by_name(factor_mapping.factor_name)
                if existing_factor:
                    return existing_factor
            
            # Create new derivative factor from IBKR tick mapping
            new_factor = DerivativeFactor(
                name=factor_mapping.factor_name,
                group=factor_mapping.factor_group,
                subgroup=factor_mapping.factor_subgroup,
                data_type=factor_mapping.data_type,
                source="IBKR",
                definition=f"{factor_mapping.description} (IBKR Tick Type {tick_type.value})"
            )
            
            # Add additional metadata from contract data if available
            if contract_data:
                if 'symbol' in contract_data:
                    new_factor.definition += f" - Symbol: {contract_data['symbol']}"
                if 'secType' in contract_data:
                    new_factor.definition += f" - Type: {contract_data['secType']}"
                if 'exchange' in contract_data:
                    new_factor.definition += f" - Exchange: {contract_data['exchange']}"
            
            # Persist to local database
            if self.local_repo:
                created_factor = self.local_repo.add(new_factor)
                if created_factor:
                    print(f"Created new derivative factor: {created_factor.name} (ID: {created_factor.id})")
                    return created_factor
            
            print(f"Failed to create derivative factor: {factor_mapping.factor_name}")
            return None
                
        except Exception as e:
            print(f"Error in get_or_create_from_tick_type for tick type {tick_type}: {e}")
            return None

    def _create_or_get(self, name: str, group: str = "derivatives", subgroup: str = "general") -> Optional[DerivativeFactor]:
        """
        Get or create a derivative factor.
        
        Args:
            name: Factor name
            group: Factor group (default: "derivatives")
            subgroup: Factor subgroup (default: "general")
            
        Returns:
            DerivativeFactor entity from database or newly created
        """
        try:
            # Check if factor already exists by name
            if self.local_repo:
                existing_factor = self.local_repo.get_by_name(name)
                if existing_factor:
                    return existing_factor
            
            # Create new derivative factor
            new_factor = DerivativeFactor(
                name=name,
                group=group,
                subgroup=subgroup,
                data_type="numeric",
                source="IBKR",
                definition=f"Derivative factor: {name} (from IBKR data)"
            )
            
            # Persist to local database
            if self.local_repo:
                created_factor = self.local_repo.add(new_factor)
                if created_factor:
                    print(f"Created new derivative factor: {created_factor.name} (ID: {created_factor.id})")
                    return created_factor
            
            print(f"Failed to create derivative factor: {name}")
            return None
                
        except Exception as e:
            print(f"Error in get_or_create for derivative factor {name}: {e}")
            return None

    def _extract_value_for_factor(self, factor_id: int, ibkr_data: Dict[str, Any]) -> Optional[Any]:
        """
        Extract derivative factor value from IBKR data.
        
        Args:
            factor_id: The factor ID
            ibkr_data: Raw IBKR data dictionary
            
        Returns:
            Extracted value or None if not available
        """
        try:
            # For derivative factors, extract various Greeks and pricing data
            if 'impliedVol' in ibkr_data:
                return float(ibkr_data['impliedVol'])
            elif 'delta' in ibkr_data:
                return float(ibkr_data['delta'])
            elif 'gamma' in ibkr_data:
                return float(ibkr_data['gamma'])
            elif 'theta' in ibkr_data:
                return float(ibkr_data['theta'])
            elif 'vega' in ibkr_data:
                return float(ibkr_data['vega'])
            elif 'rho' in ibkr_data:
                return float(ibkr_data['rho'])
            elif 'lastPrice' in ibkr_data:
                return float(ibkr_data['lastPrice'])
            elif 'bid' in ibkr_data:
                return float(ibkr_data['bid'])
            elif 'ask' in ibkr_data:
                return float(ibkr_data['ask'])
            elif 'volume' in ibkr_data:
                return int(ibkr_data['volume'])
            
            return None
            
        except (ValueError, TypeError) as e:
            print(f"Error converting derivative factor value: {e}")
            return None
        except Exception as e:
            print(f"Error extracting derivative factor value: {e}")
            return None