...
class IBKRCompanyShareMonthlyPriceRangeFactorRepository(BaseIBKRFactorRepository, CompanyShareMonthlyPriceRangeFactorPort):
    ...
    def local_repo(self):
        if self.factory:
            return self.factory._local_repositories.get('company_share_monthly_price_range_factor')
        return None
    ...
    def _create_or_get(self, name: str, **kwargs):
        try:
            ...
            print(f"Error in _create_or_get for monthly_price_range factor {name}: {e}")
            return None
    ...
    def get_by_id(self, factor_id: int) -> Optional[CompanyShareMonthlyPriceRangeFactor]:
        return self.local_repo.get_by_id(factor_id) if self.local_repo else None

    def get_by_group(self, group: str) -> List[CompanyShareMonthlyPriceRangeFactor]:
        return self.local_repo.get_by_group(group) if self.local_repo else []

    def get_by_subgroup(self, subgroup: str) -> List[CompanyShareMonthlyPriceRangeFactor]:
