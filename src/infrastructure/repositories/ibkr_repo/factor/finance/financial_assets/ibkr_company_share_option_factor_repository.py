"""
IBKR Company Share Option Factor Repository - Retrieval and creation of company share option factors via IBKR.
"""
...
class IBKRCompanyShareOptionFactorRepository(BaseIBKRFactorRepository, CompanyShareOptionFactorPort):
    """
    IBKR implementation for CompanyShareOption factor acquisition and local delegation.
    """

    def __init__(self, ibkr_client, factory=None):
        """Initialize IBKR Company Share Option Factor Repository."""
        super().__init__(ibkr_client, factory)
        self.factory = factory
        if self.factory:
            self.local_repo = self.factory._local_repositories.get('company_share_option_factor')
    ...
    def _create_or_get(self, name: str, **kwargs):
        """
        Get or create a company share option factor.
        ...
        Returns:
            CompanyShareOptionFactor entity from database or newly created
        """
        try:
            ...
            print(f"Failed to create company share option factor: {name}")
            return None
                
        except Exception as e:
            print(f"Error in get_or_create for company share option factor {name}: {e}")
            return None

    def _enhance_with_ibkr_option_data(self, primary_key: str, **kwargs) -> dict:
        """
        Enhance factor creation parameters with IBKR-specific option data for company shares.
        ...
        # Add company-specific option calculation metadata
    ...
    def _infer_underlying_symbol(self, factor_name: str) -> Optional[str]:
        ...
        if symbol_patterns:
            ...
            common_words = {'OPTION', 'CALL', 'PUT', 'PRICE', 'RETURN', 'DELTA', 'GAMMA', 'THETA', 'VEGA', 'RHO'}
