"""
IBKR Company Share Option SABR Price Factor Repository - Retrieval and creation of company share option SABR price factors via IBKR.
"""
...
from src.domain.ports.factor.finance.financial_assets.derivatives.option.company_share_option.company_share_option_sabr_price_factor_port import CompanyShareOptionSABRPriceFactorPort
from src.infrastructure.repositories.ibkr_repo.factor.base_ibkr_factor_repository import BaseIBKRFactorRepository


class IBKRCompanyShareOptionSABRPriceFactorRepository(BaseIBKRFactorRepository, CompanyShareOptionSABRPriceFactorPort):
    """
    IBKR implementation for CompanyShareOption SABR price factor acquisition and local delegation.
    """

    def __init__(self, ibkr_client, factory=None):
        """Initialize IBKR Company Share Option SABR Price Factor Repository."""
        super().__init__(ibkr_client, factory)
        self.factory = factory
        if self.factory:
            self.local_repo = self.factory._local_repositories.get('company_share_option_sabr_price_factor')
    ...
    def _create_or_get(self, name: str, **kwargs):
        """
        Get or create a company share option SABR price factor.
        ...
        Returns:
            CompanyShareOptionSABRPriceFactor entity from database or newly created
        """
        try:
            ...
            print(f"Failed to create company share option SABR price factor: {name}")
            return None
                
        except Exception as e:
            print(f"Error in get_or_create for company share option SABR price factor {name}: {e}")
            return None

    def _enhance_with_ibkr_option_data(self, primary_key: str, **kwargs) -> dict:
        """
        Enhance factor creation parameters with IBKR-specific option data for company shares using SABR model.
        ...
        Returns:
            Enhanced parameters dictionary with SABR specific metadata
        """
        ...
        if 'definition' not in enhanced:
            enhanced['definition'] = f'IBKR Company Share Option SABR Price factor: {primary_key}'
        ...
        # Add SABR model-specific parameters
        enhanced['pricing_model'] = 'sabr'
        ...
        # SABR stochastic volatility model specific enhancements
        enhanced['initial_forward_rate'] = enhanced.get('initial_forward_rate', 100.0)
        enhanced['alpha_volatility'] = enhanced.get('alpha_volatility', 0.15)
        enhanced['beta_skew'] = enhanced.get('beta_skew', 1.0)
        enhanced['rho_correlation'] = enhanced.get('rho_correlation', -0.3)
        enhanced['nu_vol_of_vol'] = enhanced.get('nu_vol_of_vol', 0.3)
        
        # Model calibration and numerical methods
        enhanced['calibration_method'] = enhanced.get('calibration_method', 'least_squares')
        enhanced['approximation_method'] = enhanced.get('approximation_method', 'hagan_formula')
        enhanced['smile_extrapolation'] = enhanced.get('smile_extrapolation', 'linear_variance')
        enhanced['monte_carlo_paths'] = enhanced.get('monte_carlo_paths', 100000)
        enhanced['time_discretization'] = enhanced.get('time_discretization', 'milstein')
        
        # Advanced SABR model features
        enhanced['displaced_diffusion'] = enhanced.get('displaced_diffusion', False)
        enhanced['free_boundary_sabr'] = enhanced.get('free_boundary_sabr', False)
        enhanced['time_dependent_parameters'] = enhanced.get('time_dependent_parameters', False)
        enhanced['volatility_smile_modeling'] = enhanced.get('volatility_smile_modeling', True)
        enhanced['skew_stickiness'] = enhanced.get('skew_stickiness', 'sticky_strike')
        
        # Market microstructure adjustments
        enhanced['bid_ask_spread_adjustment'] = enhanced.get('bid_ask_spread_adjustment', True)
        enhanced['liquidity_premium'] = enhanced.get('liquidity_premium', 0.0)
        enhanced['pin_risk_adjustment'] = enhanced.get('pin_risk_adjustment', False)
        enhanced['volatility_term_structure'] = enhanced.get('volatility_term_structure', 'time_dependent')
    ...
    def _infer_underlying_symbol(self, factor_name: str) -> Optional[str]:
        ...
        if symbol_patterns:
            ...
            common_words = {'OPTION', 'CALL', 'PUT', 'PRICE', 'RETURN', 'DELTA', 'GAMMA', 'THETA', 'VEGA', 'RHO', 'SABR'}
    ...
    def get_by_id(self, entity_id: int) -> Optional[CompanyShareOptionSABRPriceFactor]:
    ...
    def get_by_underlying_symbol(self, underlying_symbol: str) -> List[CompanyShareOptionSABRPriceFactor]:
        return self.local_repo.get_by_underlying_symbol(underlying_symbol) if self.local_repo else []

    def get_by_date_range(self, start_date: str, end_date: str) -> List[CompanyShareOptionSABRPriceFactor]:
