"""
IBKR Company Share Option Bates Price Factor Repository - Retrieval and creation of company share option Bates price factors via IBKR.
"""
...
from src.domain.ports.factor.finance.financial_assets.derivatives.option.company_share_option.company_share_option_bates_price_factor_port import CompanyShareOptionBatesPriceFactorPort
from src.infrastructure.repositories.ibkr_repo.factor.base_ibkr_factor_repository import BaseIBKRFactorRepository


class IBKRCompanyShareOptionBatesPriceFactorRepository(BaseIBKRFactorRepository, CompanyShareOptionBatesPriceFactorPort):
    """
    IBKR implementation for CompanyShareOption Bates price factor acquisition and local delegation.
    """

    def __init__(self, ibkr_client, factory=None):
        """Initialize IBKR Company Share Option Bates Price Factor Repository."""
        super().__init__(ibkr_client, factory)
        self.factory = factory
        if self.factory:
            self.local_repo = self.factory._local_repositories.get('company_share_option_bates_price_factor')
    ...
    def _create_or_get(self, name: str, **kwargs):
        """
        Get or create a company share option Bates price factor.
        ...
        Returns:
            CompanyShareOptionBatesPriceFactor entity from database or newly created
        """
        try:
            ...
            print(f"Failed to create company share option Bates price factor: {name}")
            return None
                
        except Exception as e:
            print(f"Error in get_or_create for company share option Bates price factor {name}: {e}")
            return None

    def _enhance_with_ibkr_option_data(self, primary_key: str, **kwargs) -> dict:
        """
        Enhance factor creation parameters with IBKR-specific option data for company shares using Bates model.
        ...
        Returns:
            Enhanced parameters dictionary with Bates specific metadata
        """
        ...
        if 'definition' not in enhanced:
            enhanced['definition'] = f'IBKR Company Share Option Bates Price factor: {primary_key}'
        ...
        # Add Bates model-specific parameters (extends Heston with jumps)
        enhanced['pricing_model'] = 'bates'
        ...
        # Heston components (inherited by Bates model)
        ...
        # Bates model jump-specific enhancements
        enhanced['jump_intensity'] = enhanced.get('jump_intensity', 0.1)
        enhanced['jump_size_mean'] = enhanced.get('jump_size_mean', 0.0)
        enhanced['jump_size_variance'] = enhanced.get('jump_size_variance', 0.04)
        enhanced['jump_correlation_variance'] = enhanced.get('jump_correlation_variance', 0.0)
        enhanced['compound_poisson_process'] = enhanced.get('compound_poisson_process', True)
        ...
        # Advanced Bates model features
        enhanced['variance_jump_component'] = enhanced.get('variance_jump_component', False)
        enhanced['double_exponential_jumps'] = enhanced.get('double_exponential_jumps', False)
        enhanced['jump_size_distribution'] = enhanced.get('jump_size_distribution', 'log_normal')
        enhanced['time_varying_jump_intensity'] = enhanced.get('time_varying_jump_intensity', False)
        enhanced['stochastic_interest_rates'] = enhanced.get('stochastic_interest_rates', False)
        
        # Jump risk premium modeling
        enhanced['jump_risk_premium'] = enhanced.get('jump_risk_premium', 0.0)
        enhanced['systematic_jump_risk'] = enhanced.get('systematic_jump_risk', True)
        enhanced['idiosyncratic_jump_risk'] = enhanced.get('idiosyncratic_jump_risk', False)
        enhanced['earnings_announcement_jumps'] = enhanced.get('earnings_announcement_jumps', True)
    ...
    def _infer_underlying_symbol(self, factor_name: str) -> Optional[str]:
        ...
        if symbol_patterns:
            ...
            common_words = {'OPTION', 'CALL', 'PUT', 'PRICE', 'RETURN', 'DELTA', 'GAMMA', 'THETA', 'VEGA', 'RHO', 'BATES'}
    ...
    def get_by_id(self, entity_id: int) -> Optional[CompanyShareOptionBatesPriceFactor]:
    ...
    def get_by_underlying_symbol(self, underlying_symbol: str) -> List[CompanyShareOptionBatesPriceFactor]:
        return self.local_repo.get_by_underlying_symbol(underlying_symbol) if self.local_repo else []

    def get_by_date_range(self, start_date: str, end_date: str) -> List[CompanyShareOptionBatesPriceFactor]:
