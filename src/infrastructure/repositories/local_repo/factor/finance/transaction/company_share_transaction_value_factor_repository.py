"""
Repository class for Company Share Transaction Value factor entities.
"""

from typing import Optional
from sqlalchemy.orm import Session
from src.infrastructure.repositories.mappers.factor.company_share_transaction_value_factor_mapper import CompanyShareTransactionValueFactorMapper
...
from src.domain.ports.factor.finance.transaction.company_share_transaction_value_factor_port import CompanyShareTransactionValueFactorPort


class CompanyShareTransactionValueFactorRepository(BaseFactorRepository, CompanyShareTransactionValueFactorPort):
    """Repository for Company Share Transaction Value factor entities with CRUD operations."""
    
    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanyShareTransactionValueFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        """
        Get or create a company share transaction value factor with dependency resolution.
        ...
        try:
            ...
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group', 'transaction'),
                factor_type=kwargs.get('factor_type', 'company_share_transaction_value_factor')
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition', f'company_share factor: {primary_key}')
            )
            ...
            # Create dependencies (order quantity and price that make up this transaction)
            ...
            print(f"Error in get_or_create company share transaction value factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: Optional[str] = None, subgroup: Optional[str] = None, frequency: Optional[str] = None, data_type: Optional[str] = None, source: Optional[str] = None):
        ...
        try:
            ...
            print(f"Error retrieving company share transaction value factor by all attributes: {e}")
            return None
