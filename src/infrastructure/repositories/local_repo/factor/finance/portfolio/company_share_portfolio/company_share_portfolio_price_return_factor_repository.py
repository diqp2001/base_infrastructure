...
from src.domain.ports.factor.finance.portfolio.company_share_portfolio_factor.company_share_portfolio_price_return_factor_port import CompanySharePortfolioPriceReturnFactorPort
from src.infrastructure.repositories.local_repo.factor.base_factor_repository import BaseFactorRepository
from src.infrastructure.repositories.mappers.factor.company_share_portfolio_price_return_factor_mapper import CompanySharePortfolioPriceReturnFactorMapper
from src.infrastructure.repositories.mappers.factor.factor_value_mapper import FactorValueMapper


class CompanySharePortfolioPriceReturnFactorRepository(BaseFactorRepository, CompanySharePortfolioPriceReturnFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanySharePortfolioPriceReturnFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type') or 'company_share_portfolio_price_return_factor',
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition') or f'{self.mapper.discriminator} price return factor: {primary_key}',
            )
            ...
            print(f"Error in _create_or_get CompanySharePortfolioPriceReturnFactor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name, group, factor_type=None, subgroup=None, frequency=None, data_type=None, source=None):
        try:
            ...
            print(f"Error retrieving CompanySharePortfolioPriceReturnFactor by all: {e}")
            return None
