"""
Repository class for Portfolio Company Share Correlation factor entities.
"""
...
class CompanySharePortfolioCorrelationFactorRepository(BaseFactorRepository):
    """Repository for Portfolio Company Share Correlation factor entities with CRUD operations."""
    ...
    def _to_entity(self, infra_obj):
        ...
        return FactorMapper.to_domain_portfolio_company_share_correlation_factor(infra_obj)
    ...
    def get_or_create(self, entity_cls,primary_key: str, **kwargs):
        """
        Get or create a portfolio company share correlation factor with dependency resolution.
        ...
        try:
            ...
            return self._create_or_get(
                ...
                subgroup=kwargs.get('subgroup', 'correlation'),
                data_type=kwargs.get('data_type', 'numeric'),
                source=kwargs.get('source', 'portfolio_analysis'),
                definition=kwargs.get('definition', f'Portfolio company share correlation factor: {primary_key}'),
                entity_type=kwargs.get('entity_type', 'portfolio_company_share_correlation')
            )
            
        except Exception as e:
            print(f"Error in get_or_create for portfolio company share correlation factor {primary_key}: {e}")
            return None