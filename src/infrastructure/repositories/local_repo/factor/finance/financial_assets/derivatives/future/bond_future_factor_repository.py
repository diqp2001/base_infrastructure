...
from src.infrastructure.repositories.mappers.factor.finance.financial_assets.derivatives.future.bond_future.bond_future_factor_mapper import BondFutureFactorMapper
...
class BondFutureFactorRepository(BaseFactorRepository, BondFutureFactorPort):
    """Repository for BondFutureFactor entities with CRUD operations."""

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = BondFutureFactorMapper()
    ...
    def _to_entity(self, infra_obj):
        return BondFutureFactorMapper.to_domain(infra_obj)

    def _to_model(self, entity):
        return BondFutureFactorMapper.to_orm(entity)
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type', 'bond_future_factor'),
            )
            if existing:
            ...
            print(f"Error in get_or_create for bond future factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(
        ...
        try:
            ...
            query = query.filter(FactorModel.name == name, FactorModel.group == group)
            ...
            print(f"Error retrieving bond future factor by attributes: {e}")
            return None
