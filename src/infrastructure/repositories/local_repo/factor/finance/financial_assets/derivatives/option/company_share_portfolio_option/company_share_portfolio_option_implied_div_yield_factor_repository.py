...
from src.infrastructure.repositories.mappers.factor.finance.financial_assets.derivatives.option.company_share_portfolio_option.company_share_portfolio_option_implied_div_yield_factor_mapper import CompanySharePortfolioOptionImpliedDivYieldFactorMapper
from src.infrastructure.repositories.mappers.factor.factor_value_mapper import FactorValueMapper
from src.domain.ports.factor.finance.financial_assets.derivatives.option.company_share_portfolio_option.company_share_portfolio_option_implied_div_yield_factor_port import CompanySharePortfolioOptionImpliedDivYieldFactorPort
from ......base_factor_repository import BaseFactorRepository


class CompanySharePortfolioOptionImpliedDivYieldFactorRepository(BaseFactorRepository, CompanySharePortfolioOptionImpliedDivYieldFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanySharePortfolioOptionImpliedDivYieldFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group') or 'fundamental',
                factor_type=kwargs.get('factor_type') or 'company_share_portfolio_option_implied_div_yield_factor',
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition') or f'CompanySharePortfolioOptionImpliedDivYieldFactor: {primary_key}',
            )
            ...
            print(f"Error in _create_or_get CompanySharePortfolioOptionImpliedDivYieldFactor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: str = None,
        ...
        try:
            ...
            print(f"Error retrieving CompanySharePortfolioOptionImpliedDivYieldFactor by all attributes: {e}")
            return None
