...
from src.infrastructure.repositories.mappers.factor.finance.financial_assets.derivatives.option.company_share_portfolio_option.company_share_portfolio_option_implied_vol_factor_mapper import CompanySharePortfolioOptionImpliedVolFactorMapper
from src.infrastructure.repositories.mappers.factor.factor_value_mapper import FactorValueMapper
from src.domain.ports.factor.finance.financial_assets.derivatives.option.company_share_portfolio_option.company_share_portfolio_option_implied_vol_factor_port import CompanySharePortfolioOptionImpliedVolFactorPort
from ......base_factor_repository import BaseFactorRepository


class CompanySharePortfolioOptionImpliedVolFactorRepository(BaseFactorRepository, CompanySharePortfolioOptionImpliedVolFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanySharePortfolioOptionImpliedVolFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group') or 'volatility',
                factor_type=kwargs.get('factor_type') or 'company_share_portfolio_option_implied_vol_factor',
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                name=primary_key,
                group=kwargs.get('group') or 'volatility',
                subgroup=kwargs.get('subgroup') or 'implied',
                frequency=kwargs.get('frequency') or '1d',
                data_type=kwargs.get('data_type') or 'numeric',
                source=kwargs.get('source') or 'calculated',
                definition=kwargs.get('definition') or f'CompanySharePortfolioOptionImpliedVolFactor: {primary_key}',
            )
            ...
            print(f"Error in _create_or_get CompanySharePortfolioOptionImpliedVolFactor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: str = None,
        ...
        try:
            ...
            print(f"Error retrieving CompanySharePortfolioOptionImpliedVolFactor by all attributes: {e}")
            return None
