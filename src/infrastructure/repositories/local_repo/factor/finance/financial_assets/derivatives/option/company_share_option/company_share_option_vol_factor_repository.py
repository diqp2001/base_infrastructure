...
from src.infrastructure.repositories.mappers.factor.finance.financial_assets.derivatives.option.company_share_option.company_share_option_vol_factor_mapper import CompanyShareOptionVolFactorMapper
from src.infrastructure.repositories.mappers.factor.factor_value_mapper import FactorValueMapper
from src.domain.ports.factor.finance.financial_assets.derivatives.option.company_share_option.company_share_option_vol_factor_port import CompanyShareOptionVolFactorPort
from ......base_factor_repository import BaseFactorRepository


class CompanyShareOptionVolFactorRepository(BaseFactorRepository, CompanyShareOptionVolFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanyShareOptionVolFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type') or 'company_share_option_vol_factor',
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                name=primary_key,
                group=kwargs.get('group') or 'volatility',
                subgroup=kwargs.get('subgroup') or 'realized',
                frequency=kwargs.get('frequency') or '1d',
                data_type=kwargs.get('data_type') or 'numeric',
                source=kwargs.get('source') or 'calculated',
                definition=kwargs.get('definition') or f'CompanyShareOptionVolFactor: {primary_key}',
            )
            ...
            print(f"Error in _create_or_get CompanyShareOptionVolFactor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: str = None,
        ...
        try:
            ...
            print(f"Error retrieving CompanyShareOptionVolFactor by all attributes: {e}")
            return None
