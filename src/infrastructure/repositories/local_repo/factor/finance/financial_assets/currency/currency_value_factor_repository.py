"""
Local repository for CurrencyValueFactor operations.
"""
...
class CurrencyValueFactorRepository(BaseFactorRepository, CurrencyValueFactorPort):
    """Local repository for CurrencyValueFactor entities."""

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CurrencyValueFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        """
        Get or create a CurrencyValueFactor, wiring up its dependency on
        CurrencyMidPriceFactor when creating for the first time.
        """
        try:
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group', 'value'),
                factor_type=kwargs.get('factor_type', 'currency_value_factor'),
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition', f'Market value of a Currency: {primary_key}'),
            )
            ...
            print(f"Error in _create_or_get CurrencyValueFactor '{primary_key}': {e}")
            return None
    ...
    def get_by_all(
        ...
        try:
            ...
            print(f"Error retrieving CurrencyValueFactor by all attributes: {e}")
            return None
