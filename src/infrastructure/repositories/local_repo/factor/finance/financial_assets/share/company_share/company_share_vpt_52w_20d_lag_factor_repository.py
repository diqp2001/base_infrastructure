...
from src.infrastructure.repositories.mappers.factor.finance.financial_assets.share.company_share.company_share_vpt_52w_20d_lag_factor_mapper import CompanyShareVpt52w20dLagFactorMapper
...
class CompanyShareVpt52w20dLagFactorRepository(BaseFactorRepository, CompanyShareVpt52w20dLagFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanyShareVpt52w20dLagFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                subgroup=kwargs.get('subgroup', 'trend'),
                frequency=kwargs.get('frequency'),
                factor_type=kwargs.get('factor_type', 'company_share_vpt_52w_20d_lag_factor'),
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition', f'52-week VPT with 20-day lag: {primary_key}'),
            )
            ...
            logger.error(f"Error in _create_or_get for vpt_52w_20d_lag factor {primary_key}: {e}")
            return None
