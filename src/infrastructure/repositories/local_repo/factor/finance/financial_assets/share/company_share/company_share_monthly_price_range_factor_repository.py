...
class CompanyShareMonthlyPriceRangeFactorRepository(BaseFactorRepository, CompanyShareMonthlyPriceRangeFactorPort):

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanyShareMonthlyPriceRangeFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                subgroup=kwargs.get('subgroup', 'range'),
                frequency=kwargs.get('frequency'),
                factor_type=kwargs.get('factor_type', 'company_share_monthly_price_range_factor'),
            )
            if existing:
            ...
            domain_factor = self.get_factor_entity()(
                ...
                definition=kwargs.get('definition', f'1-month price high minus 1-month price low: {primary_key}'),
            )
            ...
            logger.error(f"Error in _create_or_get for monthly_price_range factor {primary_key}: {e}")
            return None
