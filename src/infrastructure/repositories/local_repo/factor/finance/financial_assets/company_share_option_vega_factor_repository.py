"""
Repository class for Company Share Option Vega factor entities.
"""
...
class CompanyShareOptionVegaFactorRepository(BaseFactorRepository):
    """Repository for Company Share Option Vega factor entities with CRUD operations."""
    ...
    def _to_entity(self, infra_obj):
        ...
        return FactorMapper.to_domain_company_share_option_vega_factor(infra_obj)
    ...
    def get_or_create(self, entity_cls,primary_key: str, **kwargs):
        """
        Get or create a company share option vega factor with dependency resolution.
        ...
        try:
            ...
            return self._create_or_get(
                ...
                subgroup=kwargs.get('subgroup', 'option_vega'),
                data_type=kwargs.get('data_type', 'numeric'),
                source=kwargs.get('source', 'options_analysis'),
                definition=kwargs.get('definition', f'Company share option vega factor: {primary_key}'),
                entity_type=kwargs.get('entity_type', 'company_share_option_vega')
            )
            
        except Exception as e:
            print(f"Error in get_or_create for company share option vega factor {primary_key}: {e}")
            return None