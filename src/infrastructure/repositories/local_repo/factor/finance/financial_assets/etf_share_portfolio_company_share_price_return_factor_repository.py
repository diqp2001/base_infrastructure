"""
Repository class for ETF Share Portfolio Company Share Price Return factor entities.
"""
...
class ETFSharePortfolioCompanySharePriceReturnFactorRepository(BaseFactorRepository, ETFSharePortfolioCompanySharePriceReturnFactorPort):
    """Repository for ETF Share Portfolio Company Share Price Return factor entities with CRUD operations."""
    
    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = ETFSharePortfolioCompanySharePriceReturnFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        """
        Get or create an ETF share portfolio company share price return factor with dependency resolution.
        ...
        try:
            ...
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type', 'etf_share_portfolio_company_share_price_return_factor')
            )
            if existing:
            ...
            print(f"Error in get_or_create etf share portfolio company share price return factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(
        ...
        try:
            ...
            print(f"Error retrieving etf share portfolio company share price return factor by all attributes: {e}")
            return None
