"""
Repository class for ETF Share Portfolio Company Share Option Delta factor entities.
"""

from typing import Optional
from sqlalchemy.orm import Session
from src.domain.entities.factor.factor_dependency import FactorDependency
from infrastructure.repositories.mappers.factor.finance.financial_assets.share.etf_share.etf_share_portfolio_company_share_option_delta_factor_mapper import ETFSharePortfolioCompanyShareOptionDeltaFactorMapper
from src.infrastructure.repositories.mappers.factor.factor_value_mapper import FactorValueMapper
from ...base_factor_repository import BaseFactorRepository


class ETFSharePortfolioCompanyShareOptionDeltaFactorRepository(BaseFactorRepository):
    """Repository for ETF Share Portfolio Company Share Option Delta factor entities with CRUD operations."""
    
    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = ETFSharePortfolioCompanyShareOptionDeltaFactorMapper()
    ...
    def _create_or_get(self, entity_cls,primary_key: str, **kwargs):
        """
        Get or create an ETF share portfolio company share option delta factor with dependency resolution.
        ...
        try:
            ...
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group', 'etf_share_portfolio_company_share_option'),
                subgroup=kwargs.get('subgroup', 'delta'),
                factor_type=kwargs.get('factor_type', 'greek_calculation'),
                data_type=kwargs.get('data_type', 'numeric'),
                source=kwargs.get('source', 'model')
            )
            if existing:
            ...
            print(f"Error in get_or_create for ETF share portfolio company share option delta factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(
        ...
        try:
            ...
            print(f"Error retrieving ETF share portfolio company share option delta factor by all attributes: {e}")
            return None