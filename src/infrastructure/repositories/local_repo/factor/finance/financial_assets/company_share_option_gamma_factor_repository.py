"""
Repository class for Company Share Option Gamma factor entities.
"""
...
class CompanyShareOptionGammaFactorRepository(BaseFactorRepository):
    """Repository for Company Share Option Gamma factor entities with CRUD operations."""
    ...
    def _to_entity(self, infra_obj):
        ...
        return FactorMapper.to_domain_company_share_option_gamma_factor(infra_obj)
    ...
    def get_or_create(self, primary_key: str, **kwargs):
        """
        Get or create a company share option gamma factor with dependency resolution.
        ...
        try:
            ...
            return self._create_or_get(
                ...
                subgroup=kwargs.get('subgroup', 'option_gamma'),
                data_type=kwargs.get('data_type', 'numeric'),
                source=kwargs.get('source', 'options_analysis'),
                definition=kwargs.get('definition', f'Company share option gamma factor: {primary_key}'),
                entity_type=kwargs.get('entity_type', 'company_share_option_gamma')
            )
            
        except Exception as e:
            print(f"Error in get_or_create for company share option gamma factor {primary_key}: {e}")
            return None