"""
Repository class for Company Share Position Value factor entities.
"""

from typing import Optional
from sqlalchemy.orm import Session
from src.infrastructure.repositories.mappers.factor.company_share_position_value_factor_mapper import CompanySharePositionValueFactorMapper
...
from src.domain.ports.factor.finance.position.company_share_position_value_factor_port import CompanySharePositionValueFactorPort


class CompanySharePositionValueFactorRepository(BaseFactorRepository, CompanySharePositionValueFactorPort):
    """Repository for Company Share Position Value factor entities with CRUD operations."""
    
    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CompanySharePositionValueFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        """
        Get or create a company share position value factor with dependency resolution.
        ...
        try:
            ...
            existing = self.get_by_all(
                name=primary_key,
                group=kwargs.get('group', 'position'),
                factor_type=kwargs.get('factor_type', 'company_share_position_value_factor')
            )
            if existing:
            ...
            # Create dependencies (transactions that make up this position)
            ...
            print(f"Error in get_or_create company share position value factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: Optional[str] = None, subgroup: Optional[str] = None, frequency: Optional[str] = None, data_type: Optional[str] = None, source: Optional[str] = None):
        ...
        try:
            ...
            print(f"Error retrieving company share position value factor by all attributes: {e}")
            return None
