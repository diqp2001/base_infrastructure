"""
Portfolio Holding Value Factor Repository

Local repository for portfolio holding value factor entities using SQLAlchemy.
"""
...
class PortfolioHoldingValueFactorRepository(BaseFactorRepository, PortfolioHoldingValueFactorPort):
    """Local repository for portfolio holding value factor entities."""

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = PortfolioHoldingValueFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        """
        Get or create a portfolio holding value factor with dependency resolution.
        ...
        Returns:
            Portfolio holding value factor entity or None if creation failed
        """
        try:
            ...
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type', 'portfolio_holding_value_factor')
            )
            if existing:
            ...
            # Create dependencies (position value factors for holding calculation)
            ...
            print(f"Error in get_or_create portfolio holding value factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(
        ...
        try:
            ...
            print(f"Error retrieving portfolio holding value factor by all attributes: {e}")
            return None
