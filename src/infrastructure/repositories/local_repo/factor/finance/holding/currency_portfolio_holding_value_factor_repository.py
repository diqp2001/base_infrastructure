...
class CurrencyPortfolioHoldingValueFactorRepository(BaseFactorRepository, CurrencyPortfolioHoldingValueFactorPort):
    """Local repository for currency portfolio holding value factor entities."""

    def __init__(self, session: Session, factory=None):
        super().__init__(session)
        self.factory = factory
        self.mapper = CurrencyPortfolioHoldingValueFactorMapper()
    ...
    def _create_or_get(self, entity_cls, primary_key: str, **kwargs):
        try:
            existing = self.get_by_all(
                ...
                factor_type=kwargs.get('factor_type', 'currency_portfolio_holding_value_factor'),
            )
            if existing:
            ...
            print(f"Error in get_or_create currency portfolio holding value factor {primary_key}: {e}")
            return None
    ...
    def get_by_all(self, name: str, group: str, factor_type: Optional[str] = None,
        ...
        try:
            ...
            print(f"Error retrieving currency portfolio holding value factor by all attributes: {e}")
            return None
