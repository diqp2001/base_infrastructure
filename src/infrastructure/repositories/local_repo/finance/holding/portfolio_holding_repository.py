...
from infrastructure.repositories.mappers.finance.holding.portfolio_holding_mapper import PortfolioHoldingMapper
...
class PortfolioHoldingRepository:
    ...
    def get_by_id(self, holding_id: int) -> Optional[PortfolioHolding]:
        ...
        model = self.session.query(PortfolioHolding).filter_by(id=holding_id).first()
        return self.mapper.to_entity(model) if model else None

    def get_by_portfolio_id(self, portfolio_id: int) -> List[PortfolioHolding]:
        """Get all holdings for a specific portfolio"""
        models = self.session.query(PortfolioHolding).filter_by(portfolio_id=portfolio_id).all()
