...
class CompanyShareOptionVarFactor(CompanyShareOptionFactor):
    """Realized variance of a company share option price series, computed from IBKR price data.

    Resolution branch: A — depends on CompanyShareOptionFactor for the option market price
    fetched directly from IBKR.  Annualised realised variance = var(log_returns) * 252.
    """
    ...
    def calculate(self, dependencies: dict) -> Optional[float]:
        """Compute annualised realised variance from a series of option prices from IBKR."""
